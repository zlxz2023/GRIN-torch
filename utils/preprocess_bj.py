...
def create_h5_for_grin(data_dir='datasets/air_city', output_path='datasets/air_city/beijing/pm25.h5'):
    """
        创建GRIN兼容的HDF5文件
    """
    # 1. 加载站点信息
    locations_file = os.path.join(os.path.join(data_dir, 'beijing'), 'direction.csv')
    locations_df = pd.read_csv(locations_file)
    locations_df = locations_df.iloc[:, 1:4]
    sensor_count = locations_df.shape[0]
    
    # 2. 加载PM2.5数据
    pm25_file = os.path.join(os.path.join(data_dir, 'beijing'), 'pm25_bj.csv')
    ...
    # 6. 提取站点名称
    site_names = []
    if '监测点' in locations_df.columns:
        site_names = locations_df['监测点'].tolist()
    ...
    if '纬度' in locations_df.columns and '经度' in locations_df.columns and '监测点' in locations_df.columns:
        # 重命名列以匹配GRIN要求
        locations_df = locations_df.rename(columns={'监测点': 'station_name', '纬度': 'latitude', '经度': 'longitude'})
...
if __name__ == '__main__':
    create_h5_for_grin()
    pm25 = pd.read_hdf('datasets/air_city/beijing/pm25.h5', key='pm25')
    stations = pd.read_hdf('datasets/air_city/beijing/pm25.h5', key='stations')
    print(pm25)
    print(stations)
    # 统计每个站点的NaN值数量
