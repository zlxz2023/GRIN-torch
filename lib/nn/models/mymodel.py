...
class MyModel(nn.Module):
    def __init__(self,
        ...
        self.bigrill = BiGRILSemanticParallel(input_size=self.d_in,
                              ff_size=d_ff,
    ...
    def forward(self, x, mask=None, semantic_emb=None, **kwargs):
        ...
        if semantic_emb is not None:
            semantic_emb = rearrange(semantic_emb, 'b s n c -> b c n s')

        # imputation: [batches, channels, nodes, steps] prediction: [4, batches, channels, nodes, steps]
        imputation, prediction = self.bigrill(x, self.adj, mask=mask, semantic_emb=semantic_emb, cached_support=self.training)
