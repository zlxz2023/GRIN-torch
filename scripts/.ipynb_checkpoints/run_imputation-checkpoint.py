...
def get_dataset(dataset_name):
    if dataset_name[:3] == 'air':
        ...
        # 使用AirQuality类处理广州数据
        dataset = datasets.GuangzhouAirQuality(impute_nans=True, masked_sensors=[0, 1, 53, 54])
        # dataset = datasets.GuangzhouAirQuality(impute_nans=True)
    else:
...
if __name__ == '__main__':
    # args = parse_args()
    args = Namespace(adj_threshold=0.1, aggregate_by=['mean'], alpha=10.0, batch_size=32, 
                     config='config/grin/guangzhou.yaml', consistency_loss=False, d_emb=8, d_ff=64, 
                     d_hidden=64, d_in=None, d_train_freq=5, dataset_name='guangzhou', decoder_order=1,
                     delay=0, detrend=False, epochs=300, ff_dropout=0.3, g_train_freq=1, grad_clip_algorithm='norm',
                     grad_clip_val=5.0, hint_rate=0.7, horizon=24, impute_only_holes=False, in_sample=False, 
                     kernel_size=2, l2_reg=0.0, layer_norm=False, loss_fn='l1_loss', lr=0.001, merge='mlp', 
                     model_name='grin', n_layers=1, patience=10, pred_loss_weight=1, samples_per_epoch=5120, 
                     scale=True, scaled_target=True, scaling_axis='global', scaling_type='std', seed=-1, 
                     stride=1, test_len=0.2, use_lr_schedule=True, val_len=0.1, warm_up=0, whiten_prob=0.05, 
                     window=24, workers=0)
    print(args)
    run_experiment(args)
